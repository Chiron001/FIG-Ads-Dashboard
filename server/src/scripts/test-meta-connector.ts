...
// from the live Meta ad account and prints a summary.
//
// Run with: npm run meta:test --workspace server
...
async function main() {
  ...
  if (rows.length > 0) {
    ...
    console.log("\nNo rows in this date range (account may genuinely have no activity, or adsets are paused).");
  }
}

main().catch((err) => {
  console.error("\nMeta connector smoke test failed:");
  console.error(err);
  process.exit(1);
});
